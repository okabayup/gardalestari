...
import { getBeritaCategories, addBeritaCategory, deleteBeritaCategory, BeritaCategory } from '@/app/actions/berita-kategori';
...
export default function KategoriBeritaPage() {
  ...
  const [itemToDelete, setItemToDelete] = useState<BeritaCategory | null>(null);
  ...
  const onSubmit = async (data: FormData) => {
    try {
      await addBeritaCategory(data.name);
  ...
  const handleDeleteClick = (category: BeritaCategory) => {
  ...
  const handleDeleteConfirm = async () => {
    if (itemToDelete && itemToDelete.id) {
      try {
        await deleteBeritaCategory(itemToDelete.id);
    ...
    <AlertDialog open={showDeleteAlert} onOpenChange={setShowDeleteAlert}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tindakan ini akan menghapus kategori <span className="font-semibold">"{itemToDelete?.name}"</span>. Ini tidak akan menghapus berita yang menggunakan kategori ini.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
