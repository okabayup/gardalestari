...
const VideoPostClient = ({ slug }: { slug: string }) => {
    const [post, setPost] = useState<BeritaPost | null>(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        getBeritaPost(slug).then(data => {
            if (!data || data.type !== 'video' || !data.youtubeId) {
                notFound();
            } else {
                setPost(data);
            }
...
export default function VideoPostPage({ params }: Props) {
    return <VideoPostClient slug={params.slug} />;
}
