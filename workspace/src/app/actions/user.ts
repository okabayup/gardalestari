...
export async function processVerificationSubmission(
  ...
  try {
    ...
    if (!nikSnapshot.empty) {
        const existingUser = nikSnapshot.docs[0];
        // Allow if NIK belongs to the current user (e.g. re-verification)
    ...
    let newPhotoURL = userDoc.data().avatarUrl || null;
    ...
    // Generate username only if it's not set or is a temporary one
    let username = userDoc.data().username;
    if (!username || username.startsWith('anggota_') || username.startsWith('user_')) {
      username = await generateUniqueUsername(data.fullName);
    }
